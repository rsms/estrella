...
import { isWindows, jsonparseFile } from "./util"
...
export function findTSC(cwd :string) :string {
  ...
  if (npmPath) {
    ...
    if (i != -1) {
      const executable = !isWindows ? "tsc" : "tsc.cmd"
      return Path.join(npmPath.substr(0, i + find.length - Path.sep.length), ".bin", executable)
    }
  }
